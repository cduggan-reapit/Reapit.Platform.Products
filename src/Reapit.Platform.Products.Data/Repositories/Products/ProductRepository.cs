...
public class ProductRepository(ProductDbContext context) : BaseRepository<ResourceServer>(context), IProductRepository
{
    /// <inheritdoc/>
    public async Task<IEnumerable<ResourceServer>> GetProductsAsync(
        string? name = null, 
    ...
    /// <inheritdoc/>
    public async Task<ResourceServer?> GetProductByIdAsync(string id, CancellationToken cancellationToken)
        => await context.Products
